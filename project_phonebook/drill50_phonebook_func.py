...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email, col_ccourse) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com', 'Computer Applications'))
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email,col_ccourse FROM tbl_phonebook WHERE col_fullname = (?)""", [value])
...
def addToList(self):
    ...
    var_ccourse = self.txt_ccourse.get().strip()
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email,col_ccourse) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email,var_ccourse))
...
def onUpdate(self):
    try:
        ...
        return
    # The user will only be alowed to update changes for phone, emails and current courses.
    ...
    var_email = self.txt_email.get().strip()
    var_ccourse = self.txt_ccourse.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_ccourse) > 0): # ensure that there is data present
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cur = conn.cursor()
            ...
            print(count2)
            cur.execute("""SELECT COUNT(col_ccourse) FROM tbl_phonebook WHERE col_ccourse = '{}'""".format(var_ccourse))
            count3 = cur.fetchone()[0]
            print(count3)
            if count == 0 and count2 == 0 or count3 == 0: # if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel("Update Request","The following changes ({}), ({}) or ({}) will be implemented for ({}). \n\nProceed with the update request?".format(var_phone,var_email,var_ccourse,var_value))
                print(response)
                if response:
                    #conn = sqlite3.connect('db_phonebook.db')
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_phonebook SET col_phone = '{0}',col_email = '{1}',col_ccourse = '{2}' WHERE col_fullname = '{3}'""".format(var_phone,var_email,var_ccourse,var_value))
                        conn.commit()
                        onClear()
                else:
                    messagebox.showinfo("Cancel request","No changes have been made to ({}).".format(var_value))
            else:
                messagebox.showinfo("No changes detected","Both ({}), ({} and ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(var_phone, var_email, var_ccourse))
            onClear(self)
        conn.close()
    else:
