...
def describe_game(name):
    """
        ...
        If it is not a new game, thank the players for
...
def nice_mean(nice,mean,name):
    ...
    while stop:
        ...
        pick = input("\nA stranger approaches you for a \ncoversation. Will you nice \nor will you be mean? (N/M) \n>>>: ").lower()
        if pick == "n":
            print("\nThe stranger walks away smiling...")
            nice = (nice + 1)
            stop = False
        if pick == "m":
            print("\nThe  starnger glares at you \nvisibly upset, and storms off...")
            mean = (mean + 1)
            stop = False
...
def lose(nice,mean,name):
    print("\nSo sorry, Game Over! \n{}, you are now living a life with your beat up van and no friends!".format(name))
    again(nice,mean,name)
...
def reset(nice,mean,name):
    nice = 0
    mean = 0
    # Notice, do not reset teh name variable as that same user has elected to play again
