...
def home(request):
    ...
    if request.method == 'POST':
        ...
        return balance(request, pk)  # calls balance function to render said account's Balance Sheet
...
def balance(request, pk):
    ...
    current_total = account.initial_deposit  # Creates account total variable starting with the value of initial deposit
    ...
    for t in transactions:  # Loop through transactions and determine which is a deposit or withdrawal
        if t.type == 'Deposit':
            current_total += t.amount  # If deposit add amount of balance
